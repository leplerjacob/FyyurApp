...
def artists():
  ...
  #   print(artist.name)
  #   if location == artist.city + artist.state:
...
def search_artists():
  ...
  # query_artist = Artist.query.filter_by((request.form['search_term'])
  response = []

  count = 0

  artist_query = Artist.query.filter(Artist.name.like('%' + request.form['search_term'] + '%'))
